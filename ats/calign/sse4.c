...
static inline int banded_striped_epi8_seqalign_piecex_row_cal_tail_codes(
    __m128i h, __m128i u, __m128i v, i8 *us[2], int *ubegs[2]) {
  i8 vs[WORDSIZE];
  // revise the first striped block of u
  v = _mm_subs_epi8(h, u); // v(x - 1, y) = h(x - 1, y) - u(x - 1, y - 1)
  _mm_storeu_si128((__m128i *)vs, v);
  for (int i = 1; i <= WORDSIZE; i++) {
    ubegs[1][i] = ubegs[0][i] + vs[i - 1];
  }
  v = _mm_slli_si128(v, 1);
  u = _mm_loadu_si128((__m128i *)us[1]);
  u = _mm_subs_epi8(u, v); // because I previously set v to zero, now update it,
                           // u(x, y) = h(x, y) - v(x - 1, y)
  _mm_storeu_si128((__m128i *)us[1], u);
...
banded_striped_epi8_seqalign_piecex_row_cal_FPenetration_codes(i32 W, __m128i f,
                                                               int *ubegs[2],
                                                               i8 gape) {
  i8 fs[WORDSIZE];
  f = _mm_slli_si128(f, 1);
  _mm_storeu_si128((__m128i *)fs, f);
  fs[0] = I8_MIN;
...
static void banded_striped_epi8_seqalign_piecex_row_init(i8 *us, i8 *es,
                                                         ...
                                                         i8 max_nt, i8 min_nt,
                                                         i8 gapo1, i8 gape1) {
  __m128i MIN = _mm_set1_epi8(I8_MIN);
  __m128i GAP = _mm_set1_epi8(gape1);

  for (int k = 0; k < W; k++) {
    _mm_storeu_si128((__m128i *)es + k, MIN);
    _mm_storeu_si128((__m128i *)us + k, GAP);
  }
...
static inline void print_value(__m128i val) {
  char buf[WORDSIZE];
...
banded_striped_epi8_seqalign_piece1_row_cal(u8 base, i8 *us[2], i8 *es[2],
  ...
  // __m128i GapOE = _mm_set1_epi8(gapo1 + gape1);
  __m128i GapOE = _mm_set1_epi8(gapo1);
  __m128i GapE = _mm_set1_epi8(gape1);
  ...
  __m128i f = _mm_set1_epi8(I8_MIN);
  for (int i = 0; i < W; i++) {
    __m128i h = _mm_loadu_si128((__m128i *)qprof + i); // * 4);// + base);
    if (i == 0) {
      h = _mm_insert_epi8(h, h0, 0);
    }

    __m128i u = _mm_loadu_si128((__m128i *)us[0] + i);
    __m128i e = _mm_loadu_si128((__m128i *)es[0] + i);
  ...
  __m128i h = _mm_loadu_si128((__m128i *)qprof); // + base);
  __m128i v = _mm_set1_epi8(0);
  __m128i u;
  for (int i = 0; i < W; i++) {
    __m128i z = _mm_loadu_si128((__m128i *)qprof + i); // * 4);// + base);
    if (i == 0) {
      z = _mm_insert_epi8(h, h0, 0);
    }
    u = _mm_loadu_si128((__m128i *)us[0] + i);
    __m128i e = _mm_loadu_si128((__m128i *)es[0] + i);
    ...
    // calculate u(x, y)
    v = _mm_subs_epi8(h, v);
    _mm_storeu_si128((__m128i *)us[1] + i, v);
    v = _mm_subs_epi8(h, u);
    ...
    _mm_storeu_si128((__m128i *)es[1] + i, e);
  ...
  // __m128i last = _mm_loadu_si128((__m128i *)us[0] + W - 1);
  return banded_striped_epi8_seqalign_piecex_row_cal_tail_codes(h, u, v, us,
                                                                ubegs);
}
...
do_the_thing(u8 *qseq, i64 unaligned,
  ...
  i64 score =
      banded_striped_epi8_seqalign_getscore(u1, ubegs1, W, unaligned-1);
  return score;
}
